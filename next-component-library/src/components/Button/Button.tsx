...
export const Button: React.FC<ButtonProps> = ({ onClick, role, children, ...props }) => {
  const buttonStyleByRole =
    role === 'buttonPip'
      ? 'button-pip flex justify-center items-center border-none p-2.5'
  ...
  return (
    <button className={buttonStyleByRole} onClick={onClick} {...props}>
      {children}
    </button>
  );
};
